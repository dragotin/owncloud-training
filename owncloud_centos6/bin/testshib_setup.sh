...
cd /var/www/owncloud
su apache -s /bin/bash -c 'php occ app:enable user_shibboleth'
su apache -s /bin/bash -c 'php occ shibboleth:mode ssoonly'

echo "create testshib users"
su apache -s /bin/bash -c 'OC_PASS=myself php occ user:add myself@testshib.org --password-from-env'
su apache -s /bin/bash -c 'OC_PASS=alterego php occ user:add alterego@testshib.org --password-from-env'
su apache -s /bin/bash -c 'OC_PASS=superego php occ user:add superego@testshib.org --password-from-env'

curl -k -XPOST https://$ADMIN:$PASSWORD@oc.docker/owncloud/ocs/v1.php/cloud/users/myself@testshib.org/groups -d groupid="admin"
